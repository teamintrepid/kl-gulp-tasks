...
gulp.task('test', function () {
  ...
  .on('end', function () {
    if (!errorHappened) {
      notifier.notify(
        { title: 'All tests passed', message: 'Write some more' }
      );
    }
  });
});
...
gulp.task('compile-and-test', function (callback) {
  sequence('ts-compile', 'test')(callback);
});
gulp.task('ts-fresh-compile', function (callback) {
  sequence('cleanup', 'ts-compile')(callback);
});
gulp.task('ts-run-build', function (callback) {
  sequence('ts-fresh-compile', 'watch-compile')(callback);
});
